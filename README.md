...
7. Shows administrators a compact support summary for the topic author directly below the topic metadata, beside every topic on the Recent page, and on each user's profile. The corresponding APIs are protected by a server-side administrator check and never return license keys or email addresses.
